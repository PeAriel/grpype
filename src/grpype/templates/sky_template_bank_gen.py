...
def generate_integration_sky_params(nangs, params_path):
    params_path = resolve_path(params_path, ptype='templates')
    os.makedirs(params_path, exist_ok=True)

    params = np.zeros([180, 3])
    i = 0
    for epeak in np.logspace(np.log10(10), np.log10(3000), 12):
        for alpha in np.linspace(-1.5, 3, 5):
            for beta in np.linspace(-5, -1.6, 3):
...
def generate_integration_sky_templates(
    nangs,
    ref_date,
    responses_path=None,
    bank_path=INTEGRATION_SKY_FOLDER,
):
    """
    Generate integration sky templates.

    Args:
        nangs (int): Number of angles for the integration grid
        ref_date (datetime): Reference date
        responses_path (str): Path to the responses, if None, will generate the responses in the same path as the bank parameters
    ...
    generate_integration_sky_params(nangs, bank_path)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate integration sky templates")
    ...
    print(f'Generating integration sky templates for {ref_date} using {args.nangs} angles')

    generate_integration_sky_templates(
        nangs=args.nangs,
        ref_date=ref_date,
        bank_path=INTEGRATION_SKY_FOLDER,
